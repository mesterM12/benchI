...
import { member, runs, submittedTrials } from "../../../../lib/server";

export function createPost(authenticate: typeof member, trials: SubmittedTrials, orchestration: Pick<RunOrchestration, "submissionSlotForTrial">) {
  return async (request: Request) => {
    ...
    try {
      const bundle = await request.json() as SubmissionBundle;
      if (await orchestration.submissionSlotForTrial(bundle.trialId) !== bundle.submissionSlotId) return Response.json({ code: "UNKNOWN_SUBMISSION_SLOT" }, { status: 422 });
      const result = await trials.publish(bundle, actorId, idempotencyKey, new Date().toISOString());
...
export const POST = createPost(member, submittedTrials, runs);
